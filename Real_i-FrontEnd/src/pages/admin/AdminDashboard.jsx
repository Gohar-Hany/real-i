...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-6 lg:space-y-8 animate-fade-in-up pb-10">
        ...
        <div className="relative z-10 p-8 sm:p-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            ...
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-surface-50 dark:text-surface-100 tracking-tight mb-3 font-heading">
              Welcome back, <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary-500 via-primary-600 to-amber-700 dark:from-primary-300 dark:via-primary-400 dark:to-primary-500">{user?.name?.split(' ')[0] || 'Admin'}</span>
            </h1>
            <p className="text-surface-400 text-sm sm:text-base max-w-2xl leading-relaxed font-sans">
          ...
          <div className="flex gap-3 shrink-0">
            <button
              onClick={checkHealth}
              className="flex items-center gap-2 px-5 py-3 rounded-xl bg-surface-800 text-surface-50 dark:text-surface-200 border border-surface-600/40 dark:border-surface-700 hover:bg-surface-700/80 transition-all active:scale-95 shadow-sm cursor-pointer"
            >
              <Activity size={16} className="text-emerald-500" />
              <span className="font-bold text-sm font-sans">Diagnostics</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {health && (
        <div className="relative rounded-3xl p-6 bg-surface-900 border border-surface-600/40 dark:border-surface-700/50 shadow-md overflow-hidden animate-fade-in">
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-500 to-blue-500"></div>
          <h3 className="text-base font-bold text-surface-50 dark:text-surface-100 mb-4 flex items-center gap-3 font-heading">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {Object.entries(health).map(([key, value]) => {
              ...
              return (
                <div key={key} className="flex items-center gap-3 p-3 rounded-xl bg-surface-800/40 border border-surface-600/30 dark:border-surface-700/50">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${isUp ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-rose-500/10 text-rose-600 dark:text-rose-400'}`}>
                    {isUp ? <CheckCircle size={16} /> : <XCircle size={16} />}
                  </div>
                  <div className="min-w-0">
                    <p className="text-[10px] font-bold text-surface-400 uppercase tracking-wider font-mono">{key.replace(/_/g, ' ')}</p>
                    <p className={`text-xs font-semibold truncate ${isUp ? 'text-surface-50 dark:text-surface-200' : 'text-danger-text font-bold'}`}>
          ...
          >
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-2xl font-extrabold text-surface-50 dark:text-surface-100 font-heading">{stat.value}</span>
              </div>
              <p className="text-xs font-bold text-surface-50 dark:text-surface-300 uppercase tracking-wider font-mono">{stat.label}</p>
              <p className="text-[10px] text-surface-400 mt-0.5 font-sans">{stat.sub}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="flex items-center gap-3 mb-5">
            ...
            <h2 className="text-lg font-bold text-surface-50 dark:text-surface-100 font-heading">Control Modules</h2>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {quickActions.map((action, i) => (
              <button
                ...
                className="relative bg-surface-900 rounded-2xl p-5 text-left border border-surface-600/40 dark:border-surface-700/50 shadow-sm hover:border-primary-500 hover:shadow-md transition-all duration-300 group hover:-translate-y-1 cursor-pointer"
              >
                <div className="flex items-start gap-4 relative z-10">
                  <div
                    className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0 border shadow-sm"
                    style={{ background: `linear-gradient(135deg, ${action.color}20, ${action.color}05)`, borderColor: `${action.color}40` }}
                  >
                    <action.icon size={20} style={{ color: action.color }} />
                  </div>
                  <div className="flex-1 pt-0.5">
                    <p className="text-sm font-bold text-surface-50 dark:text-surface-100 mb-1 group-hover:text-primary-600 dark:group-hover:text-primary-300 transition-colors font-heading">{action.label}</p>
                    <p className="text-[11px] text-surface-400 leading-relaxed font-sans">{action.desc}</p>
                  </div>
                  <div className="w-7 h-7 rounded-full bg-surface-800 border border-surface-600/30 dark:border-surface-700 flex items-center justify-center shrink-0 group-hover:bg-primary-500 group-hover:text-surface-950 text-surface-400 transition-all mt-0.5">
        ...
        <div className="rounded-3xl border border-surface-600/40 dark:border-surface-700/50 bg-surface-900 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-surface-600/30 dark:border-surface-800 bg-surface-800/40 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-blue-500/10 flex items-center justify-center border border-blue-500/20">
                <GraduationCap size={16} className="text-blue-500 dark:text-blue-400" />
              </div>
              <h3 className="text-sm font-bold text-surface-50 dark:text-surface-100 font-heading">Recent Students</h3>
            </div>
            <Link to="/admin/students" className="text-[10px] text-primary-600 dark:text-primary-400 font-bold hover:text-primary-500 uppercase tracking-wider flex items-center gap-1 font-mono">
          ...
          <div className="p-3 space-y-1.5">
            {loading ? (
              <div className="py-8 text-center">
                ...
                <p className="text-[10px] text-surface-400 uppercase tracking-widest font-mono">Loading...</p>
              </div>
            ) : recentStudents.length === 0 ? (
              <div className="py-8 text-center">
                <Users size={24} className="text-surface-400 mx-auto mb-2" />
                <p className="text-xs text-surface-400 font-sans">No students found</p>
              </div>
            ) : (
              recentStudents.map((s, i) => (
                <Link
                  ...
                  className="flex items-center gap-3 p-3 rounded-xl hover:bg-surface-800/60 transition-colors group"
                >
                  <div className="w-9 h-9 rounded-lg bg-gradient-to-r from-primary-400 via-primary-500 to-primary-600 flex items-center justify-center text-surface-950 text-xs font-extrabold shrink-0 shadow-sm">
                    {s.name?.charAt(0)?.toUpperCase() || '?'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-surface-50 dark:text-surface-200 truncate group-hover:text-primary-600 dark:group-hover:text-primary-300 transition-colors font-heading">{s.name}</p>
                    <p className="text-[10px] text-surface-400 font-mono truncate">{s.email}</p>
                  </div>
                  <Eye size={14} className="text-surface-400 group-hover:text-primary-500 shrink-0 transition-colors" />
                </Link>
              ))
            )}
          </div>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        {[
          ...
          <div key={i} className={`bg-surface-900 rounded-2xl p-6 border border-surface-600/40 dark:border-surface-700/50 hover:border-primary-500/50 shadow-sm transition-all group`}>
            <div className="w-10 h-10 rounded-xl bg-surface-800 flex items-center justify-center border border-surface-600/30 dark:border-surface-700 mb-4 shadow-sm">
              <card.icon size={20} className="text-primary-600 dark:text-primary-400" />
            </div>
            <h3 className="text-sm font-bold text-surface-50 dark:text-surface-100 mb-2 font-heading">{card.title}</h3>
            <p className="text-xs text-surface-400 leading-relaxed mb-4 font-sans">{card.desc}</p>
            <Link
              to={card.link}
              className="inline-flex items-center gap-1.5 text-[11px] font-bold text-primary-600 dark:text-primary-400 hover:text-primary-500 transition-colors uppercase tracking-wider font-mono"
            >

      ...
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
        ...
        <div className="relative flex-1 sm:max-w-xs ml-auto">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-surface-900/60 border border-surface-700/50 text-sm text-surface-50 placeholder-surface-500 focus:outline-none focus:border-primary-500/50 focus:ring-1 focus:ring-primary-500/50 transition-all"
          />
        </div>
      </div>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between p-4 border-t border-surface-700/50 bg-surface-900/30">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => setCurrentPage(1)} disabled={currentPage === 1} className="p-2 rounded-lg bg-surface-800/50 border border-surface-700 text-surface-400 hover:text-surface-50 disabled:opacity-30 disabled:cursor-not-allowed transition-all"><ChevronsLeft size={14} /></button>
                  <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-2 rounded-lg bg-surface-800/50 border border-surface-700 text-surface-400 hover:text-surface-50 disabled:opacity-30 disabled:cursor-not-allowed transition-all"><ChevronLeft size={14} /></button>
                  {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                    ...
                    return (
                      <button key={page} onClick={() => setCurrentPage(page)} className={`w-8 h-8 rounded-lg text-xs font-bold transition-all ${currentPage === page ? 'gradient-primary text-surface-950' : 'bg-surface-800/50 border border-surface-700 text-surface-400 hover:text-surface-50'}`}>{page}</button>
                    );
                  })}
                  <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-2 rounded-lg bg-surface-800/50 border border-surface-700 text-surface-400 hover:text-surface-50 disabled:opacity-30 disabled:cursor-not-allowed transition-all"><ChevronRight size={14} /></button>
                  <button onClick={() => setCurrentPage(totalPages)} disabled={currentPage === totalPages} className="p-2 rounded-lg bg-surface-800/50 border border-surface-700 text-surface-400 hover:text-surface-50 disabled:opacity-30 disabled:cursor-not-allowed transition-all"><ChevronsRight size={14} /></button>
                </div>
              </div>
            )}
          </>

const NODE_API_BASE = (import.meta.env.VITE_API_URL || '/api/v1').replace(/\/+$/, '');
const AI_API_BASE = (import.meta.env.VITE_AI_URL || import.meta.env.VITE_API_URL || '/api/v1').replace(/\/+$/, '');

class ApiClient {
  getBaseUrl(endpoint) {
    const isAiEndpoint = /^\/(agent|nlp|data|admin)/.test(endpoint);
    return isAiEndpoint ? AI_API_BASE : NODE_API_BASE;
  }

  async request(endpoint, options = {}, retries = 2, backoff = 1000) {
    const base = this.getBaseUrl(endpoint);
